...
# Suppress the home energy pop up setting up fresh Sonoma macs. Example https://imgur.com/a/ZcMruTp
...
launchctl asuser $uid sudo -u ${currentUser} "$@" /usr/bin/defaults write com.apple.Home showHomeEnergy -bool false
sleep 3

if [[ launchctl asuser "$uid" sudo -u "${currentUser}" "$@" $shestatus == "0" ]]; then
